...
us.ret <- read.csv(paste0(path,"/GithubFiles/F2US.csv"), check.names = F) %>% `rownames<-`(NULL)
uk.ret <- read.csv(paste0(path,"/GithubFiles/F2UK.csv"), check.names = F) %>% `rownames<-`(NULL)

us.w.ben <- read.csv(paste0(path,"/GithubFiles/F3US.csv"), check.names = F) %>% `rownames<-`(NULL)
uk.w.ben <- read.csv(paste0(path,"/GithubFiles/F3UK.csv"), check.names = F) %>% `rownames<-`(NULL)

us.w.inst <- read.csv(paste0(path,"/GithubFiles/F4USInst.csv"), check.names = F) %>% `rownames<-`(NULL)
uk.w.inst <- read.csv(paste0(path,"/GithubFiles/F4UKInst.csv"), check.names = F) %>% `rownames<-`(NULL)

us.w.slow <- read.csv(paste0(path,"/GithubFiles/F4USSlow.csv"), check.names = F) %>% `rownames<-`(NULL)
uk.w.slow <- read.csv(paste0(path,"/GithubFiles/F4UKSlow.csv"), check.names = F) %>% `rownames<-`(NULL)
...
gdp <- read.csv(paste0(path,"/data/gdpfin.csv")) %>% `rownames<-`(NULL)
gdp.port <- merge(gdp, us.port)
gdp.port <- merge(gdp.port, uk.port)
write.csv(gdp.port, paste0(path,"/data/gdpandport.csv"), row.names = F)
...
gdp.port.fund <- read.csv(paste0(path,"/data/gdpandport.csv"))
